...
class ProductInfo:
    ...
    PRODUCT_CATEGORY = (
        _ZNJJ, _HEALTHY, _SPORT, _SECURITY, _KITCHEN
    )
    ...
    def setCategory(self, category):
        if category not in ProductInfo.PRODUCT_CATEGORY:
            self.product_category = ProductInfo.PRODUCT_CATEGORY[0]
        else:
    ...
    def downloadImg(self):
        ...
        if len(self.product_cover_img) > 0:
            ...
            WeissUtils.requestUrlContent(self.product_cover_img, os.path.join(self.img_cache_dir, str(self.product_id)))

        for i in self.product_thumbnail:
            # downloadFile(i, str(self.product_id))
            WeissUtils.requestUrlContent(i, os.path.join(self.img_cache_dir, str(self.product_id)))
    ...
    def downloadHtml(self, db=None, mysqlDB=None):
        ...
        WeissUtils.requestUrlContent(url, self.html_cache_dir, cache_file_name)
        ...
        if db:
            CREATE_PRODUCT_VIEW_TABLE = 'CREATE TABLE IF NOT EXISTS "products_view" ("product_id" INTEGER PRIMARY KEY  NOT NULL  UNIQUE, "product_intro" TEXT, "product_detail" TEXT, "product_thumbnails" TEXT, "buy_url" TEXT, "tags" TEXT , "category_id" TEXT)'
            try:
                ...
                db.execute(INSERT_COMMAND, (self.product_id, _product_intro, product_detail, str(product_thumbnails), buy_url, "", self.product_category))
...
if __name__ == "__main__":
    ...
    p.setCategory(ProductInfo._SPORT)
    p.downloadHtml(db)